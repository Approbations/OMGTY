...
for (int i = 0; i < steps; i++)
{
    ...
    if (dct.ContainsKey(number[1]))
    {
        dct[number[1]] += int.Parse(number[3]);
        int sum = int.Parse(hashtable[number[1]].ToString());
        hashtable.Remove(number[1]); hashtable.Add(number[1], int.Parse(number[3]) + sum);
    }
    else
    {
        dct[number[1]] = int.Parse(number[3]);
        hashtable.Add(number[1], int.Parse(number[3]));
    }
}
